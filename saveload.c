...
void LoadNewConfig(TabInt *T, Graf *G, Peta *P)
/* I.S. : A, G, P kosong */
...
void SaveConfig(TabInt T, Graf G, Peta P, FLAGS F)
...
void LoadExistingConfig(TabInt *T, Graf *G, Peta *P, FLAGS *F1, FLAGS *F2)
...
{   /* Algoritma */
    ...
    for (i = 1; i <= JmlhBang; i++) {
        ...
        ADVDATA();
        psk = CLData.Value;
        B = MakeBangunan(kep, psk, a, m, u, lvl, x, y, jns);
        AddAsLastEl(*T, B);
        ADVDATA();
    }
}
...
{   /* Kamus lokal */
    ...
    jmlhbang = Neff(T);
    for (i = 1; i <= jmlhbang; i++) {
        fprintf(src, "%c %d %d %d %d %d %d %d %d\n", jenis(bangunan(T, i)), Absis(bangunan(T, i)), Ordinat(bangunan(T, i)), kepemilikan(bangunan(T, i)), level(bangunan(T, i)), tambahpas(bangunan(T, i)), maks(bangunan(T, i)), pasawal(bangunan(T, i)), pasukan(bangunan(T, i)));
    }
}
